...
import com.ec2.joaquin.servicio.ClientesServicio;
...
public class ClienteControlador {
	...
	public ResponseEntity<List<ClienteDTOResponse>> listarClientes() {
		return new ResponseEntity<List<ClienteDTOResponse>>(servicio.listarClientes(), HttpStatus.OK);
	}
	...
	public ResponseEntity<ClienteDTOResponse> listarPorId(@PathVariable Integer id) {
		...
		if (p != null)
			return new ResponseEntity<ClienteDTOResponse>(p, HttpStatus.OK);
	...
	public ResponseEntity<Void> editar(@RequestBody ClienteDTORequest cliente) {
		...
		ClienteDTOResponse p = servicio.obtenerClienteId(cliente.getIdClienteDTO());
